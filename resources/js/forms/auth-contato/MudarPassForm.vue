        ...
        <q-card-section>
          ...
          >
            ...
            <q-input
              ...
              bottom-slots
              :error="
                errors && errors.confirmation && errors.confirmation.length > 0
              "
            >
              <template v-slot:error>
...
export default {
  ...
  computed: {
    passwordConfirmationRule() {
      return () =>
        this.password === this.confirmation || "Senhas devem combinar";
    },
  },
  methods: {
    async onSubmit() {
      ...
      try {
        let { data } = await axios.post(
          "/auth/modificar-senha/" + this.$route.params.token,
          form
        );
        if (data.success) {
    ...
    async validaToken() {
      try {
        let { data } = await axios.get(
          "/auth/verificar/" + this.$route.params.token
        );
      } catch (ex) {
        this.errors = ex.errors;
        this.$router.push(`/usuario/login`);
