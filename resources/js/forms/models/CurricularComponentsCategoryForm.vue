  ...
  <div class="row q-pa-md">
    <div class="col-sm-12">
      <q-card flat bordered>
        <h5 v-if="curricularComponentCategory.id != null">
          Edição da Categoria Componente Curricular
          <b>{{ this.categoryName }}</b>
        </h5>
        <h5 v-if="curricularComponentCategory.id == null">
          Cadastro da Categoria Componente Curricular
        </h5>
        <form v-on:submit.prevent="save()">
          <ShowErrors :errors="errors.name"></ShowErrors>
          <q-input
            filled
            v-model.trim="curricularComponentCategory.name"
          ...
          <q-item-label style="margin-bottom:5px"
            ...
            /><strong>Componentes</strong></q-item-label
          >
          <q-select
            class="col-sm-10"
            filled
  ...
  methods: {
    async save() {
      ...
      if (this.curricularComponentCategory.componentes.length) {
        let componentes = this.curricularComponentCategory.componentes.map(
          (item) => item.id
        );
    ...
    getComponentes(val, update, abort) {
      update(() => {
        if (val === "" && val.length < 2) {
          ...
          const self = this;
          axios
            .get(`componentescategorias/autocomplete/${val}`)
            .then((resp) => {
    ...
    async getComponenteCategoria() {
      ...
      let resp = await axios.get(
        `/componentescategorias/${this.$route.params.id}`
      );
