  ...
  <div class="row q-pa-md">
    <div class="col-sm-6">
      <form v-on:submit.prevent="save()">
        <q-input
          filled
          v-model.trim="canal.options.extend_name"
          label="Nome do canal em extenso"
          hint="Nome do canal em extenso"
        ></q-input>
        <q-input
          filled
          v-model.trim="canal.name"
          label="Nome do Canal"
        ...
        <q-input
          filled
          v-model.trim="canal.options.back_url"
          label="URL da API"
          hint="BackEnd URL"
        ></q-input>
        <q-select
    ...
    <div class="col-sm-6">
      <q-card flat bordered class="q-ml-sm">
        <q-card-section>
          <p v-if="canal.id">
            ID: <strong>{{ canal ? canal.id : null }}</strong>
          </p>
          <p>
            URL AMIGÁVEL: <strong>{{ url }}</strong>
          </p>
          <p>
            NOME: <strong>{{ canal.name }}</strong>
          </p>
          <p>
            NOME EM EXTENSO: <strong>{{ canal.options.extend_name }}</strong>
          </p>
          <p>
            ATIVO: <strong>{{ formatTrueFalse(canal.is_active) }}</strong>
          </p>
          <p>
            POSSUI HOME:
            <strong>{{ formatTrueFalse(canal.options.has_home) }}</strong>
          </p>
          <p>
            POSSUI SOBRE:
            <strong>{{ formatTrueFalse(canal.options.has_about) }}</strong>
          </p>
          <p>
            POSSUI CATEGORIAS:
            <strong>{{ formatTrueFalse(canal.options.has_categories) }}</strong>
          </p>
          <p>
            POSSUI ACESSO RÁPIDO:
            <strong>{{
              formatTrueFalse(canal.options.has_quick_access)
            }}</strong>
          </p>
          <p>
            TIPOS DE CONTEÚDOS:
            <strong v-for="(tipo, t) in canal.tipos" :key="t">
              {{ tipo.name }}
            </strong>
          </p>
        </q-card-section>
        <q-card-section v-if="canal.options">
          ...
          <p>
            O QUÊ?: <span>{{ canal.options.complement_description.que }}</span>
          </p>
          <p>
            O PORQUÊ:
            <span>{{ canal.options.complement_description.porque }}</span>
          </p>
          <p>
            O CÔMO: <span>{{ canal.options.complement_description.como }}</span>
          </p>
        </q-card-section>
      </q-card>
    </div>
...
export default {
  ...
  methods: {
    async save() {
      ...
      this.canal.options.tipo_conteudo = this.canal.tipos
        ? this.canal.tipos.map((i) => i.id)
        : [];
