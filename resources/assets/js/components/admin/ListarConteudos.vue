<template>
  <div class="q-pa-md">
    ...
    <div class="col-lg-12 flex flex-center q-gutter-sm">
      <!-- ADICIONAR -->
      <q-btn icon="add" color="positive" size="xs" :to="`/admin/conteudos/adicionar`" title="Adicionar novo item"
        label="adicionar item" />
      <!-- LISTAR CONTEÚDOS APROVADOS -->
      <q-btn icon="checked" color="primary" size="xs" label="Aprovar" @click="getAprovados()" />
      <q-btn color="primary" icon="person" size="xs" label="Meus Conteúdos" @click="getOwnerResources()" />
      <q-space></q-space>
      <!-- PAGINAÇÃO -->
      <q-pagination v-if="paginator && paginator.total > paginator.per_page" v-model="paginator.current_page"
        :max="paginator.last_page" :input="true" @input="getConteudos">
      </q-pagination>
    ...
    <div class="col-lg-12 q-mt-xs" v-if="paginator && paginator.total > 0">
      <!-- LISTA -->
      <q-markup-table separator="horizontal" flat bordered :dense="$q.screen.lt.md">
        <thead>
          <tr>
        ...
        <tbody>
          <tr v-for="conteudo in paginator.data" :key="conteudo.id" :id="`item-${conteudo.id}`">
            <td class="text-center" v-html="conteudo.title"></td>
            ...
            <td class="text-center" style="width:50px;">
              <q-btn-group spread>
                <q-btn size="sm" color="primary" title="Editar item" icon="edit"
                  v-if="conteudo.user_can && conteudo.user_can.update" :to="`/admin/conteudos/editar/${conteudo.id}`" />
                <q-btn size="sm" color="negative" title="Deletar item"
                  @click="deleteItem(`/conteudos/${conteudo.id}`, conteudo.id)" icon="delete"
                  v-if="conteudo.user_can && conteudo.user_can.delete" />
                <q-btn size="sm" color="primary" title="Visualizar" icon="visibility" type="a" target="__blank"
    ...
    <div class="col-lg-12 q-mt-lg" v-if="paginator && paginator.total > paginator.per_page">
      ...
      <div class="flex flex-center">
        <q-pagination v-model="paginator.current_page" :max="paginator.last_page" :max-pages="10" boundary-numbers
          @input="getConteudos">
        </q-pagination>
      </div>
    </div>
