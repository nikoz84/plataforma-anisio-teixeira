...
<div class="q-pa-md row justify-center q-gutter-xs">
    <q-card class="col-sm-6">
      <q-card-section class="q-gutter-sm">
        <q-input :loading="loadingState" clearable debounce="300" 
          label="Pesquisar conteúdo por título" v-model="term">
        ...
        <q-list bordered separator>
            ...
            <q-item @click="selectItem(conteudo)" 
              clickable v-ripple
              v-for="conteudo in conteudos" 
              :key="conteudo.id" :id="conteudo.id">
              <q-item-section>
      ...
      <q-card class="col-sm-5">
        ...
        <q-card-section v-if="conteudo && conteudo.title">
          ...
          <div v-if="conteudo && conteudo.tipo">Tipo: {{ conteudo.tipo.name }}</div>
          <q-separator></q-separator>
        ...
        <q-card-actions>
          <q-btn label="Editar" color="primary" :to="`/admin/conteudos/editar/${conteudo.id}`"></q-btn>
        </q-card-actions>
      </q-card>
...
import { PlayListModalForm } from "@forms/models";
...
export default {
    ...
    computed:{
      ...mapState(["conteudo"]),
      hasSite(){
        return this.conteudo && this.conteudo.options ? this.conteudo.options.site : null
      }
    },    
    methods: {
      ...
      async search()
      {
        ...
        console.log(params)
        const { data } = await axios.get(`/playlist/search/${this.term}`,{ params });
